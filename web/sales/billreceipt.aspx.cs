...
public partial class billreceipt : System.Web.UI.Page
{
    ...
    public static string showBillReceipt(string billno)
    {
        ...
        try
        {
            ...
            String orderQry = "SELECT distinct cu.cust_id, sm.sm_id,sm.sm_refno, cu.cust_name,concat(ud.first_name,' ',ud.last_name) as staff_name,cu.cust_address as address,cu.cust_city as city,cu.cust_country as country,cust_reg_id as gst,cu.cust_state as state,cu.cust_phone as phone,cu.cust_email as email "
                 ...
                 + " FROM tbl_sales_master sm inner join tbl_customer cu on cu.cust_id=sm.cust_id right join tbl_transactions tr on (tr.action_ref_id=sm.sm_id and tr.action_type=" + (int)Constants.ActionType.SALES + " ) left join tbl_state on tbl_state.state_id=cu.cust_state"
                 + "  left join tbl_user_details ud on ud.user_id=sm.sm_userid inner join tbl_sales_items tsi on tsi.sm_id=sm.sm_id WHERE sm.sm_id ='" + billno + "' group by tsi.si_id";
            ...
            String qryItems = "SELECT tsi.itm_code,tsi.itm_name,si_qty as qty,si_price as price,si_foc as foc "
                + " ,si_total,si_discount_rate as dis_rate,si_discount_amount as dis_amount"
                + " ,si_tax_excluded_total as taxableValue,si_net_amount as net_amount,tp_tax_code,si_item_cgst as cgst,si_item_sgst as sgst,si_item_igst as igst "
                + " FROM tbl_sales_items tsi inner join tbl_itembranch_stock tis on tis.itbs_id=tsi.itbs_id WHERE sm_id='" + dtOrder.Rows[0]["sm_refno"].ToString() + "' and si_itm_type!=2";
