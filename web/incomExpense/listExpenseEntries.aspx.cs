...
public partial class listExpenseEntries : System.Web.UI.Page
{
    ...
    public static string searchEntries(int page, Dictionary<string, string> filters, int perpage)
    {
        ...
        try
        {
            ...
            string query_condition = " where tie.ie_type='0' ";
            if (filters.Count > 0)
            {

                if (filters.ContainsKey("search"))
                {
                    query_condition += " and (ie_id = '" + filters["search"] + "' or ie_invoice_num = '" + filters["search"] + "' or ext_user_name like '%" + filters["search"] + "%')";
                }
            ...
            //countQry = "SELECT count(*) as Count FROM tbl_incm_exps where ie_type='" + (int)Constants.ActionType.EXPENSE + "'";
            countQry = "SELECT count(*) as Count FROM tbl_incm_exps tie inner join tbl_user_details tu on tu.user_id=tie.ext_user_id " + query_condition;
            ...
            innerqry = "select ie_id,ie_netamount as net_amount,ie_invoice_num,ie_cat_name as ie_category,ext_user_name as externalusername,"
                + " DATE_FORMAT(ie_date, '%d-%b-%Y') AS ieDate, sum(cr) - sum(dr) as ie_total_balance,concat(tu.first_name,tu.last_name) as enteredusername, tu.user_id as enteredUserId"
                ...
                + " inner join tbl_transactions tr on (tr.action_ref_id=tie.ie_id and tr.action_type=" + (int)Constants.ActionType.EXPENSE + ") " + query_condition + "";
