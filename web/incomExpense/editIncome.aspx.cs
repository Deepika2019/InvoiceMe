...
public partial class incomExpense_editIncome : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        LoginAuthentication income = new LoginAuthentication();
        income.userAuthentication();
        income.checkPageAcess(55);
    }
    ...
    public static string retrieveData(string incomeId)
    {
        ...
        //Query to Push Payment Details and vendor details as Header
                                       //tu.user_name
        string previouspaidqry = "select ext_user_name as ext_user_name,ext_user_id as ext_user_id,tie.branch_id,ie_balance,ie_netamount as total_amount,sum(dr)-sum(cr) as ie_total_balance,ie_netamount-(sum(dr)-sum(cr)) as ie_total_paid,DATE_FORMAT(ie_date,'%d-%b-%Y') AS date,ie_note,ie_invoice_num from tbl_incm_exps tie"
    ...
    [WebMethod]
    public static string updateIncomeEntry(Dictionary<string, string> filters)
    {
        ...
        try
        {
            ...
            updatePurchaseMasterQry = updatePurchaseMasterQry + "ie_netamount  ='" + filters["TotalNetAmount"] + "',ie_tax  ='" + filters["totalTaxamt"] + "', ";
            updatePurchaseMasterQry = updatePurchaseMasterQry + "ie_invoice_num  ='" + filters["invoicenum"] + "' ";
            ...
            if (db.ExecuteQueryForTransaction(updatePurchaseMasterQry))
            {
                ...
                if (Convert.ToDouble(filters["TotalNetAmount"]) > old_netamount)
                {
                    ...
                    cmdInsCr.Parameters.AddWithValue("@narration", "Reconciliation of Income entry #" + filters["invoicenum"] + " after an edit by crediting the amount " + (Convert.ToDouble(filters["TotalNetAmount"]) - old_netamount));
                    cmdInsCr.Parameters.AddWithValue("@dr", (Convert.ToDouble(filters["TotalNetAmount"]) - old_netamount));
                ...
                else if (Convert.ToDouble(filters["TotalNetAmount"]) < old_netamount)
                {
                    ...
                    cmdInsDr.Parameters.AddWithValue("@narration", "Reconciliation of Income entry #" + filters["invoicenum"] + " after an edit by debiting the amount " + (old_netamount - Convert.ToDouble(filters["TotalNetAmount"])));
                    cmdInsDr.Parameters.AddWithValue("@cr", (old_netamount - Convert.ToDouble(filters["TotalNetAmount"])));
    ...
    public static void savePaymentEdit(int trans_id, double cash_amt, double cheque_amt)
    {
        ...
        try
        {
            ...
            if (new_total > prev_total)
            {
                ...
                cmdInsDr.Parameters.AddWithValue("@narration", "Reconciliation of payment #" + trans_id + " of income entry #" + dt.Rows[0]["action_ref_id"].ToString() + " after edit by debiting the amount " + (new_total - prev_total));
            ...
            else
            {
                ...
                cmdInsCr.Parameters.AddWithValue("@narration", "Reconciliation of payment #" + trans_id + " of income entry #" + dt.Rows[0]["action_ref_id"].ToString() + " after edit by crediting the amount " + (prev_total - new_total));
            ...
            string update_vendor_qry = "UPDATE tbl_vendor SET vn_balance=(select (sum(dr)-sum(cr)) from tbl_transactions where partner_id='" + dt.Rows[0]["partner_id"].ToString() + "' and partner_type=" + (int)Constants.PartnerType.COMMONUSER + ") WHERE vn_id='" + dt.Rows[0]["partner_id"].ToString() + "'";
            bool upvndr_result = db.ExecuteQueryForTransaction(update_vendor_qry);
    ...
    public static string updateTransaction(Dictionary<string, string> filters)
    {
        ...
        string updateTransaction = "UPDATE tbl_transactions set cash_amt='" + filters["cashAmt"] + "',cheque_amt='" + filters["chequeAmt"] + "',cheque_no='" + filters["chequeNo"] + "',cheque_date='" + filters["popupChequeDate"] + "',cheque_bank='" + filters["chequeBank"] + "',cr='" + filters["totalAmt"] + "',narration='Credited " + filters["totalAmt"] + " Against Income Entry #" + filters["invoiceNum"] + "'" + query_condition;
