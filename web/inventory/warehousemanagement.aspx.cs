...
public partial class inventory_warehousemanagement : System.Web.UI.Page
{
    ...
    {
        LoginAuthentication warehouseManagement = new LoginAuthentication();
        warehouseManagement.userAuthentication();
        warehouseManagement.checkPageAcess(13);
    }
    //show branches
    [WebMethod]
    ...
    [WebMethod]
    public static string addBranchStockDetails(string actionType, string itbsid, string branch, string item, int currentstock, string reorderlevel, string status, string itm_mrp, string pricegroup_one, string pricegroup_two, string pricegroup_three, string taxcode, string duration, int priority, string sessionId, string actual_stock, int stockChange, string stockDifr, string purchasePrice, string purchaseMaintainStatus)
    {
        ...
        if (actionType == "update")
        {
           ...
           // dt2= db.SelectQuery("select itm_code,itm_name from tbl_item_master where itm_id='" + item + "'");
            ...
            + " itm_subcategory_id=(select itm_subcategory_id from tbl_item_master where itm_id=" + item + "),itbs_purchase_status="+purchaseMaintainStatus+"";
    ...
    public static string searchBranchStock(int page, Dictionary<string, string> filters, int perpage)
    {
        ...
        try
        {
            ...
            if (filters.Count > 0)
            {
                ...
                if (filters.ContainsKey("priceType"))
                {
                    if (Convert.ToInt32(filters["priceType"]) == 1)
                    {
                        query_condition += " and itbs.itm_class_one>0";
                    }
                    else if (Convert.ToInt32(filters["priceType"]) == 2)
                    {
                        query_condition += " and itbs.itm_class_two>0";
                    }
                    else if (Convert.ToInt32(filters["priceType"]) == 3)
                    {
                        query_condition += " and itbs.itm_class_three>0";
                    }
                }
            }
            ...
            innerqry = @"select itbs.itbs_id,itbs.itm_code,itbs.itm_name,branch_name,itbs_reorder,itbs_available,itm_class_one as classA,itm_class_two as classB,itm_class_three as classC,
...
on itbs.itbs_id = si.itbs_id  and sm.sm_delivery_status in (0,3,6) ";
    ...
    //search branchstock table
    [WebMethod]
    public static string DownloadItemDetails(Dictionary<string, string> filters)
    {
        ...
        try
        {
            ...
            if (filters.Count > 0)
            {
                ...
                if (filters.ContainsKey("priceType"))
                {
                    ...
                    {
                        query_condition += " and itbs.itm_class_one>0";
                        selectCondition += " itbs.itm_class_one as classA";
                    }
                    else if (Convert.ToInt32(filters["priceType"]) == 2)
                    {
                        query_condition += " and itbs.itm_class_two>0";
                        selectCondition += " itbs.itm_class_two as classB";
                    }
                    else if (Convert.ToInt32(filters["priceType"]) == 3)
                    {
                        query_condition += " and itbs.itm_class_three>0";
                        selectCondition += " itbs.itm_class_three as classC";
                    }
                }
                else
                {
                    selectCondition += " itbs.itm_class_one as classA,itbs.itm_class_two as classB,itbs.itm_class_three as classC";
                }
            }
            ...
            innerqry = @"select itbs.itm_code as 'Item Code',itbs.itm_name as 'Item Name',branch_name as 'Warehouse',brand_name as Brand,cat_name as Category,
tp_tax_title as 'HSN Code',tp_tax_percentage as 'Tax Percentage'," + selectCondition;
            innerqry += " from tbl_itembranch_stock itbs inner join tbl_tax_profile tp on tp.tp_tax_code=itbs.tp_tax_code inner join tbl_item_brand ib on ib.brand_id=itbs.itm_brand_id inner join tbl_item_category ic on ic.cat_id=itbs.itm_category_id inner join tbl_branch br on br.branch_id=itbs.branch_id  left join tbl_sales_items si";
innerqry += "  inner join tbl_sales_master sm on sm.sm_id = si.sm_id on itbs.itbs_id = si.itbs_id  and sm.sm_delivery_status in (0,3,6) ";
            innerqry = innerqry + query_condition;
            innerqry = innerqry + " group by itbs.itbs_id order by itbs.itbs_id";
